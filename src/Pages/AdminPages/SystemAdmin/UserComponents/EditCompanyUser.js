...
function EditUser(props) {
    ...
    const [username, setUsername] = useState(props.location.data.user.username);
    const [firstName, setFirstName] = useState(props.location.data.user.first_name);
    const [lastName, setLastName] = useState(props.location.data.user.last_name);
    const [email, setEmail] = useState(props.location.data.user.email);
  ...
  const handleEditUser = async () => {
      ...
      if (response.status === 200){
        props.history.push({
          pathname: "/admin/clients/details/" + userCompanyId,
        });
      }
      // await editUser(id, username, firstName, lastName, email, primaryPhone, secondaryPhone, roleName, subRoleName)
      // props.history.push({
      //   pathname: "/admin/clients/details/" + userCompanyId,
      // });
    }
    useEffect(() => {
    console.log('USER_EDIT_PROPS:', props)
    setUserCompanyId(props.location.companyData)
    }, [])
